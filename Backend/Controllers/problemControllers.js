...
export const getProblems_user = async (req, res) => {
  try {
    // Query problems with visibility=true, excluding 'testcases' field
    const problems = await Problem.find({ visibility: true }).select("-testcases");
    res.json(problems);
  } catch (err) {
